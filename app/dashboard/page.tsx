      ...
      <div className="space-y-8">
        ...
        <div className="border-2 border-dashed border-indigo-200 bg-white rounded-3xl p-16 text-center">
          ...
          <p className="text-gray-500 mb-8 max-w-sm mx-auto">
            Add your exam subjects and start tracking countdowns right away.
          </p>
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            {upcoming.length} upcoming • {critical.length} critical
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Next Exam In', value: nextExam ? `${Math.max(0, differenceInDays(startOfDay(new Date(nextExam.exam_date + 'T00:00:00')), today))}d` : '—', icon: '⚡', color: 'bg-purple-50 border-purple-100', text: 'text-purple-700' },
        ].map(s => (
              ...
              <div className={`relative h-full bg-white rounded-3xl overflow-hidden shadow-sm border border-gray-100 card-hover ${isPast ? 'opacity-60' : ''}`}>
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between mb-4">
                    <h3 className="font-bold text-gray-900 text-base leading-snug line-clamp-2 flex-1 mr-3" title={subject.name}>
                      {subject.name}
                    </h3>
                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold border shrink-0 ${badgeBg}`}>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-2 text-sm text-gray-500">
                      ...
                      <span className="font-medium text-gray-700">{format(examDate, 'MMMM d, yyyy')}</span>
                    </div>
                    ...
                    {subject.syllabus_text && (
                      <div className="flex items-start gap-2 text-sm text-gray-500">
                        ...
                        <p className="line-clamp-2 text-gray-500">{subject.syllabus_text}</p>
                      </div>
                    )}
                ...
                {/* Hover Edit overlay */}
                <div className="absolute inset-0 bg-indigo-600/0 group-hover:bg-indigo-600/5 transition-all rounded-3xl flex items-center justify-center opacity-0 group-hover:opacity-100">
                  <span className="text-xs font-semibold text-indigo-600 bg-white border border-indigo-200 px-3 py-1 rounded-full shadow-sm">
                    Click to Edit
                  </span>
                </div>
              </div>
